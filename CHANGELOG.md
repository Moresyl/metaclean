...
- The visible queue now shares the native 10,000-file ceiling. Repeated imports
  report entries that do not fit instead of growing an unprocessable queue that
  would fail only when scanning starts.

- Windows candidate preflight was rerun after the queue-boundary hardening;
  NSIS, MSI, portable and all 113 per-user Explorer command checks passed
  without publishing.

- Queue mutations now share one synchronized state path so rapid import,
  removal and clear actions cannot overwrite one another between renders.

- The Windows candidate preflight was rerun after the final queue-state cleanup;
  all package and Explorer integration smoke checks passed without publishing.

- Windows candidate preflight was rerun on the browser-folder identity fix;
  NSIS, MSI, portable and all 113 per-user Explorer command checks passed
  without publishing.
...
- Final Windows candidate preflight was rerun against the source content
  committed as `b8f5f48` after the streaming intake hardening; it remains
  non-publishing.
...
- Audit export now preflights its 10 MiB UTF-8 payload budget and checks queue
  liveness after asynchronous version lookup, preventing oversized IPC writes
  and post-unmount exports.
...
- 命令面板高亮现在只在搜索结果或可执行性真正变化时校正；父页面进度重绘不会打断正在进行的键盘选择。
- 命令面板高亮校正还绑定稳定命令 ID；列表在相同位置替换操作时不会把旧高亮误投到新命令。
- 命令面板搜索只命中禁用操作时仍会展示结果，但不会把它暴露为可执行的活动命令或读屏高亮。
- About 页版本说明打开失败现在会进入页面错误提示，不再产生未处理 Promise 或静默失败。
- 路径展开和目录定位在窗口卸载前后都检查生命周期，快速切页/退出不会再启动迟到的原生导入或外部文件夹调用。
