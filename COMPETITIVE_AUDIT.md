...
| Actual application intake | README lists 90+ ExifTool writer formats, but `SUPPORTED_EXTENSIONS` in `src/domain/files/file_types.ts` is a 30-entry whitelist and both drop/folder paths enforce it | 113 extensions traverse the real application intake, classification, shell integration and tests, and every one of the baseline's 30 is inside them | Exceeds; strict superset of the competitor's real intake |
...
to 113 extensions without routing unknown binary formats through a generic
...
| File formats | PNG, JPEG, WebP, AVIF, HEIC, BMP, GIF, TIFF, SVG, PDF, DOCX, XLSX, PPTX, EPUB, ODT, HTML, Markdown, MP4/MOV/M4A/M4V, WAV, MP3 | All of the above are inside the 113-extension allowlist, alongside JPEG XL, the full ODF family, 23 camera raw formats, AVI, ASF/WMV, Matroska/WebM, FLAC, AIFF and common CSS/INI/TOML configuration files | Exceeds; strict superset |
