...
export default function CleanOptions({ mode, onModeChange, disabled, scanned, busy, onAction }: CleanOptionsProps) {
  ...
  return (
    <aside className="options-card">
      ...
      <button className="scan-button" type="button" disabled={disabled || busy} onClick={onAction}><ScanSearch size={17} />{busy ? text("处理中…", "Working…") : scanned ? text("确认并开始清理", "Confirm and clean") : text("扫描隐私痕迹", "Scan privacy traces")}</button>
