...
interface CleanOptionsProps { mode: CleanMode; onModeChange: (mode: CleanMode) => void; preserveTimestamps: boolean; onPreserveTimestampsChange: (value: boolean) => void; preserveOrientation: boolean; onPreserveOrientationChange: (value: boolean) => void; disabled: boolean; scanned: boolean; hasFindings: boolean; busy: boolean; onAction: () => void }

export default function CleanOptions({ mode, onModeChange, preserveTimestamps, onPreserveTimestampsChange, preserveOrientation, onPreserveOrientationChange, disabled, scanned, hasFindings, busy, onAction }: CleanOptionsProps) {
  const { text } = useI18n();
  return (
    <aside className="options-card">
      ...
      <div className="scan-summary"><span>{text("将检测", "Scans for")}</span><ul><li>EXIF / GPS</li><li>{text("音频标签与封面", "Audio tags and artwork")}</li><li>{text("文档作者与修订", "Document authors and revisions")}</li><li>PDF {text("属性与 XMP", "properties and XMP")}</li><li>{text("不可见 Unicode", "Invisible Unicode")}</li></ul></div>
