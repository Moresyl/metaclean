...
describe("desktop components", () => {
  ...
  it("renders queue findings, errors and removal controls", () => {
    ...
    wrap(<FileQueue entries={entries} preserveColorProfile removeExtendedAttributes={false} onRemove={onRemove} onClear={vi.fn()} onReveal={vi.fn()} />);
  ...
  it("renders empty and every queue lifecycle status", () => {
    const { rerender } = wrap(<FileQueue entries={[]} preserveColorProfile removeExtendedAttributes={false} onRemove={vi.fn()} onClear={vi.fn()} onReveal={vi.fn()} />);
    ...
    rerender(<I18nProvider><FileQueue entries={entries} preserveColorProfile removeExtendedAttributes={false} onRemove={vi.fn()} onClear={vi.fn()} onReveal={vi.fn()} /></I18nProvider>);
  ...
  it("sorts the queue stably and reveals cleaned output with its size delta", () => {
    ...
    const { container } = wrap(<FileQueue entries={entries} preserveColorProfile removeExtendedAttributes={false} onRemove={vi.fn()} onClear={vi.fn()} onReveal={onReveal} />);
