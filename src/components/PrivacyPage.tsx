...
export default function PrivacyPage() {
  ...
  return (
    <section className="secondary-page">
      ...
      <div className="scope-card"><h2>{text("当前支持范围", "Supported scope")}</h2><p>{text("JPEG、PNG、WebP、GIF 图片元数据；MP3、WAV、FLAC 音频标签；MP4、MOV、M4V、M4A 视频用户数据、XMP 与位置；DOCX、XLSX、PPTX、ODT 文档属性及批注；PDF 属性和 XMP；TXT、Markdown、HTML、SVG 等文本中的不可见字符与生成器元数据。", "JPEG, PNG, WebP and GIF metadata; MP3, WAV and FLAC audio tags; MP4, MOV, M4V and M4A user data, XMP and location; DOCX, XLSX, PPTX and ODT properties/comments; PDF properties and XMP; invisible Unicode and generator metadata in TXT, Markdown, HTML and SVG files.")}</p><p>{text("统计型文本水印、像素域水印和传统二进制 Office 格式不在处理范围内。", "Statistical text watermarks, pixel-domain watermarks and legacy binary Office formats are out of scope.")}</p></div>
    </section>
  );
}
