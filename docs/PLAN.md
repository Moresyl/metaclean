...
1. 建立真实大批量基准：不同文件大小、嵌套目录、慢盘和失败混合批次，记录吞吐、峰值内存、首个结果时间和尾部延迟。当前已加入不触碰用户文件的 release-only native benchmark（128 个嵌套文本夹具，4 KiB/64 KiB/512 KiB 混合，输出 scan/clean 吞吐与清理 p95）；慢盘和失败混合仍待专门机器夹具。
2. 评估有限并发和背压；只有在不破坏源快照、写入顺序和隐私事件边界时才启用并发。可取消清理已实现，但仍需在大批量基准中验证取消延迟。
...
2. 将安装包、更新源、签名、回滚和失败恢复做成可重复的本地 preflight，避免仅凭 CI 绿灯宣称用户机器可用。
