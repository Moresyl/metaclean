...
3. 为文本/配置扩展建立独立的编码、超长行、BOM、换行、只读和恶意嵌套样例矩阵。UTF-8（含/不含 BOM）与 BOM 标记的 UTF-16 LE/BE、CRLF 保留、无效代理项、1 MiB 单行输入和超过递归预算的 SVG 已覆盖；零宽字符先规范化再匹配 HTML 结构化元数据。只读介质仍需专门机器夹具，且不把它误称为已完成。
