...
export function validateReleaseNotes(tag, body) {
  ...
  for (const heading of REQUIRED_HEADINGS) {
    ...
    const nextHeading = contentStart < 0 ? -1 : normalized.indexOf("\n### ", contentStart);
    const section = contentStart < 0 ? "" : normalized.slice(contentStart, nextHeading < 0 ? undefined : nextHeading);
    if (!/^\s*-\s+\S/mu.test(section)) throw new Error(`Release notes section is missing concrete bullets: ${heading}`);
  }
  if (/仅见|see changelog|changelog only/iu.test(normalized)) throw new Error("Release notes cannot delegate all detail to the changelog");
