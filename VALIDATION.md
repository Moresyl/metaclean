...
- Frontend: 251 tests. Statements 88.98%, branches 81.46%, functions 90.72%, lines 91.40%. The queue suite covers audit export as well as sorting, size deltas, per-file and batch path copying, errors and reveal actions; status-bar state/version, count-only batch progress, stale progress-event isolation, cancellable cleanup, crash-safe interrupted-batch recovery markers, Windows path-alias reconciliation, serialized updater checks/installs with fail-closed reviewed-version handling, focus-contained dialogs, lazy page boundaries, support links and current localized scope counts are also exercised.
