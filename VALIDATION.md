...
- Rust: 174 tests total: 171 pass and three explicit tests are ignored by default: the two native batch benchmarks plus the external Office compatibility test, the latter requiring `METACLEAN_OFFICE_SAMPLE_DIR` with real fixtures. Scan and cleanup batch registries reject duplicate tokens and support file-boundary cancellation; scan progress callbacks are regression-tested without exposing paths through the desktop event. Text cleaners round-trip UTF-8 with/without BOM and BOM-marked UTF-16 LE/BE while preserving encoding, BOM and CRLF; malformed UTF-16 surrogate data is rejected before output allocation. Strict Clippy passes with warnings denied.
...
- Local Windows release run on 2026-09-12 produced: 128 files / 25,014,272 payload bytes; scan 1,200.47 files/s; clean 149.84 files/s; first cleanup result 1.17 ms; cleanup p95 16.89 ms. The mixed 128-item run (96 valid, 32 failed) produced scan 1,187.90 files/s; clean 193.10 files/s; first result 2.23 ms; result p95 16.82 ms. These are workstation baselines, not hardware-independent service-level objectives.
