...
- Frontend: 249 tests. Statements 88.70%, branches 81.20%, functions 90.72%, lines 91.15%. The queue suite covers audit export as well as sorting, size deltas, per-file and batch path copying, errors and reveal actions; status-bar state/version, count-only batch progress, stale progress-event isolation, cancellable cleanup, crash-safe interrupted-batch recovery markers, Windows path-alias reconciliation, updater flows, focus-contained dialogs, lazy page boundaries, support links and current localized scope counts are also exercised.
- Rust: 168 tests total: 166 pass and two explicit tests are ignored by default: the native batch benchmark plus the external Office compatibility test, the latter requiring `METACLEAN_OFFICE_SAMPLE_DIR` with real fixtures. Strict Clippy passes with warnings denied.
...
- Frontend regression tests cover update-dialog focus containment/restoration, direct queue path copying, duplicate-safe native batch handling, delayed progress-event isolation, cleanup cancellation and path-free interrupted-batch recovery; recovery progress is throttled off the hot path and native close requests are blocked while an active scan or cleanup task is registered.
...
- `pnpm test:release` runs 33 checks proving version metadata, the 1180 × 720 caption/status layout and tag-specific bilingual notes stay synchronized; package/updater collection, signatures, AppImage zsync, GUI subsystem, `latest.json` and SHA-256 manifests are validated before publication.
...
- npm's official audit endpoint reports no known dependency vulnerability. Vitest is pinned to 4.1.11, and the WebdriverIO/Mocha `js-yaml` chain is forced to 4.3.2. WebdriverIO's unfixed `extract-zip` 2.0.1 dependency is replaced on every path by the repository-owned `vendor/extract-zip` 2.0.2 package, which rejects out-of-root symlink targets. Its `deepmerge-ts` chain is forced to 8.0.0 for CVE-2026-40345. `pnpm test:supply-chain` proves both the archive escape and recursive-object denial-of-service regressions are closed. Patched `glob` and `serialize-javascript` versions are also forced through workspace overrides.
...
- A repeatable native batch benchmark is available as `pnpm test:benchmark`; it creates 128 nested temporary text fixtures in release mode, mixes 4 KiB/64 KiB/512 KiB inputs, reports scan and clean throughput plus first-file and p95 cleanup latency, and removes the temporary directory automatically. It is intentionally opt-in and does not claim slow-disk or mixed-failure results.
- Local Windows release run on 2026-09-12 produced: 128 files / 25,014,272 payload bytes; scan 1,384.02 files/s; clean 162.89 files/s; first cleanup result 1.00 ms; cleanup p95 14.94 ms. These are a workstation baseline, not a hardware-independent service-level objective.
...
- `scripts/preflight-windows.ps1` is the repeatable non-publishing Windows candidate gate: it builds the debug NSIS/MSI pair, packages x64 portable output, then invokes all three installer/portable smoke checks. It never pushes, tags or creates a release.
