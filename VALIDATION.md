...
- Frontend: 252 tests. Statements 89.00%, branches 81.55%, functions 90.72%, lines 91.41%. The queue suite covers audit export as well as sorting, size deltas, per-file and batch path copying, errors and reveal actions; status-bar state/version, count-only batch progress, stale progress-event isolation, cancellable cleanup, crash-safe interrupted-batch recovery markers, Windows path-alias reconciliation, serialized updater checks/installs with fail-closed reviewed-version handling and stale-version clearing after failed re-checks, focus-contained dialogs, lazy page boundaries, support links and current localized scope counts are also exercised.
