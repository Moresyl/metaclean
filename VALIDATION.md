...
- `scripts/preflight-windows.ps1` is the repeatable non-publishing Windows candidate gate: it builds the debug NSIS/MSI pair, packages x64 portable output, then invokes all three installer/portable smoke checks. It never pushes, tags or creates a release; MSI smoke refuses to replace an existing MetaClean installation and preserves its logs when a validation step fails.
