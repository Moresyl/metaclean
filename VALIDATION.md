...
- Frontend: 246 tests. Statements 88.86%, branches 81.59%, functions 90.70%, lines 91.38%. The queue suite covers audit export as well as sorting, size deltas, per-file and batch path copying, errors and reveal actions; status-bar state/version, count-only batch progress, stale progress-event isolation, cancellable cleanup, crash-safe interrupted-batch recovery markers, updater flows, focus-contained dialogs, lazy page boundaries, support links and current localized scope counts are also exercised.
