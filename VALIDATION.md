...
- Frontend: 104 tests. Statements 94.49%, branches 87.90%, functions 94.96%, lines 97.67%. Every supported extension has its own case-insensitive classification case.
- Rust: 53 regular tests plus one ignored external Office compatibility test. Core regions 81.47%, lines 80.89% after excluding Tauri/platform glue (`lib`, `main`, data-only `models` and `shell_integration`) from the line threshold; cleaners, intake, engine and safe I/O remain included. Every ISO media and UTF-8 text alias traverses scan and clean through the public engine path.
- Installed desktop E2E: 4 WebdriverIO scenarios pass against E2E-featured Windows, macOS and Linux/Xvfb binaries, covering startup/navigation, all 26 locale options and Arabic RTL, theme persistence across a real webview reload, and the Rust `scan_files` IPC boundary. GitHub CI #21 validated the 47-extension branch, 104 frontend cases, the base gate and all three desktop jobs successfully in 5m40s: `https://github.com/Moresyl/metaclean/actions/runs/32034874179`.
