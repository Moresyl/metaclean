...
- Frontend: 256 tests. Statements 89.06%, branches 81.75%, functions 90.74%, lines 91.50%. The queue suite covers audit export as well as sorting, size deltas, per-file and batch path copying, errors and reveal actions; status-bar state/version, count-only batch progress, cancellable scanning and cleanup, incremental retry without discarding completed reports, stale progress-event isolation, crash-safe interrupted-batch recovery markers, Windows path-alias reconciliation, serialized updater checks/installs with fail-closed reviewed-version handling, stale-version clearing after failed re-checks and direct signed installation from the update prompt, focus-contained dialogs, lazy page boundaries, support links and current localized scope counts are also exercised.
- Rust: 173 tests total: 170 pass and three explicit tests are ignored by default: the two native batch benchmarks plus the external Office compatibility test, the latter requiring `METACLEAN_OFFICE_SAMPLE_DIR` with real fixtures. Scan and cleanup batch registries reject duplicate tokens and support file-boundary cancellation. Text cleaners round-trip UTF-8 with/without BOM and BOM-marked UTF-16 LE/BE while preserving encoding, BOM and CRLF; malformed UTF-16 surrogate data is rejected before output allocation. Strict Clippy passes with warnings denied.
