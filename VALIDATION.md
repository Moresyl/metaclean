...
| M3: Windows release | Complete for v0.2.0; current branch pending next release | Optimized v0.2.0 NSIS and MSI packages build locally and were published by the successful GitHub release matrix. The current release workflow script was also run locally against a freshly rebuilt NSIS: it installed to an isolated temporary directory, remained active for six seconds with the `MetaClean` title, then uninstalled with the executable absent. The current branch expands both installer cleanup paths to 47 extensions and still requires a new tagged artifact before that expansion is considered shipped. |
...
- Frontend: 109 tests. Statements 94.20%, branches 86.56%, functions 95.17%, lines 97.50%. Every supported extension has its own case-insensitive classification case; queue tests cover stable sorting, size deltas, cleanup errors and output reveal.
- Rust: 59 regular tests plus one ignored external Office compatibility test. Core regions 80.53%, lines 80.03% after excluding Tauri/platform glue (`lib`, `main`, data-only `models` and `shell_integration`) from the line threshold; cleaners, intake, engine and safe I/O remain included. Every ISO media and UTF-8 text alias traverses scan and clean through the public engine path, and public cleanup results assert exact source/output byte counts.
- Installed desktop E2E: 5 WebdriverIO scenarios cover startup/navigation, all 26 locale options and Arabic RTL, theme and ICC preference persistence across real webview reloads, and the Rust `scan_files` IPC boundary. CI #25 passed all five scenarios on Windows, Ubuntu and macOS in 5m04s: `https://github.com/Moresyl/metaclean/actions/runs/32038686443`; the same five scenarios also passed locally after the queue changes.
...
- `pnpm test:release` proves tag-specific bilingual notes contain every required section, release assets are collected without omissions/duplicates, and SHA-256 manifests are deterministic, complete and self-excluding. Each platform installs or copies and launch-smokes its packaged application before uploading an internal artifact; only after all four builds pass does the final job create the public release with `SHASUMS256.txt`. Windows NSIS is locally proven end to end; macOS/Linux scripts are syntax-checked but await the next tagged matrix run.
