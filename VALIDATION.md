...
- Rust: 171 tests total: 168 pass and three explicit tests are ignored by default: the two native batch benchmarks plus the external Office compatibility test, the latter requiring `METACLEAN_OFFICE_SAMPLE_DIR` with real fixtures. Scan and cleanup batch registries reject duplicate tokens and support file-boundary cancellation. Strict Clippy passes with warnings denied.
