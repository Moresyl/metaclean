...
| Text, markup & configuration | `.txt`, `.md`, `.markdown`, `.html`, `.htm`, `.xhtml`, `.svg`, `.xml`, `.json`, `.csv`, `.tsv`, `.yaml`, `.yml`, `.log`, `.srt`, `.vtt`, `.css`, `.scss`, `.less`, `.ini`, `.conf`, `.cfg`, `.toml`, `.properties` | UTF-8 input is rewritten only after contextual Unicode checks; Markdown/HTML/SVG additionally remove applicable front-matter or generator traces and inspect bounded embedded images. Malformed or non-UTF-8 input is refused, and decoded nested images are independently re-inspected. |
...
UTF-8 text, markup and configuration extensions. CI rejects any mismatch between those
manifests.
