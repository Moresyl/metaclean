...
| Text, markup & configuration | `.txt`, `.md`, `.markdown`, `.html`, `.htm`, `.xhtml`, `.svg`, `.xml`, `.json`, `.csv`, `.tsv`, `.yaml`, `.yml`, `.log`, `.srt`, `.vtt`, `.css`, `.scss`, `.less`, `.ini`, `.conf`, `.cfg`, `.toml`, `.properties` | UTF-8 (with or without BOM) and BOM-marked UTF-16 LE/BE are rewritten after contextual Unicode checks, preserving the original encoding, BOM and line endings; Markdown YAML/TOML front matter and HTML/SVG generator traces are removed only in their applicable bounded regions, and embedded images are inspected within explicit limits. Malformed or non-BOM text encodings are refused, and decoded nested images are independently re-inspected. |
