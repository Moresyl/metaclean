...
| Text, markup & configuration | `.txt`, `.md`, `.markdown`, `.html`, `.htm`, `.xhtml`, `.svg`, `.xml`, `.json`, `.csv`, `.tsv`, `.yaml`, `.yml`, `.log`, `.srt`, `.vtt`, `.css`, `.scss`, `.less`, `.ini`, `.conf`, `.cfg`, `.toml`, `.properties` | UTF-8 input is parsed and rewritten only after the contextual Unicode, front-matter, generator and embedded-image checks pass; malformed or non-UTF-8 input is refused, and decoded nested images remain bounded and independently re-inspected. |
